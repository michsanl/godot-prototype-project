...
var enemy_focused_slot_controller: DiceSlotController
var enemy_focused_slot_index: int
...
func _ready() -> void:
	...
	EventBus.enemy_focus_selection_added.connect(_on_enemy_focus_selection_added)
	EventBus.enemy_focus_selection_removed.connect(_on_enemy_focus_selection_removed)
	#EventBus.enemy_highlight_selection_added.connect()
	#EventBus.enemy_highlight_selection_removed.connect()


#region Enemy Methods
func _on_enemy_focus_selection_added(controller: DiceSlotController, index: int):
	enemy_focused_slot_controller = controller
	enemy_focused_slot_index = index
	
	if focused_slot_controller != null and ability_index >= 0:
		focused_slot_controller.select_slot_target(focused_slot_index, index, controller)
		enemy_focused_slot_controller.unfocus_slot(enemy_focused_slot_index)
		
		ability_index = -1
		focused_slot_controller = null
		focused_slot_index = -1
		enemy_focused_slot_controller = null
		enemy_focused_slot_index = -1
		view.hide()

func _on_enemy_focus_selection_removed(controller: DiceSlotController, index: int):
	focused_slot_controller = null
	focused_slot_index = -1


#func _on_enemy_highlight_selection_added(controller: DiceSlotController, index: int):
...
#
#
#func _on_enemy_highlight_selection_removed(controller: DiceSlotController, index: int):
	#if focused_slot_controller != null:
		#return
	#
	#view.hide()
...
func _on_remove_ability_button_pressed():
	if focused_slot_controller != null:
		focused_slot_controller.unselect_slot_ability(focused_slot_index)
